            ...
            <div class="col-12">
              <form class="form-label-left input_mask" action="{{ url('/dashboard/daftar_mandor_update') }}" method="post">
                ...
                <div class="form-group row">
                  ...
                  <div class="col-md-9 col-sm-9 ">
                    <input name="alamat" class="form-control" placeholder="Masukkan Alamat Mandor" value="{{ old('alamat') ?? $mandor->alamat }}">
                    @error('alamat')
                      <small>*{{ $message }}</small>
                    @enderror
                  </div>
                ...
                <div class="form-group row">
                  ...
                  <div class="col-md-9 col-sm-9 ">
                    <input type="text" class="form-control" placeholder="Masukkan Nomor HP Mandor"
                      name="telp" value="{{ old('telp') ?? $mandor->hp }}">
                    @error('telp')
                      <small>*{{ $message }}</small>
                    @enderror
                  </div>
                ...
                <div class="form-group row">
                  ...
                  <div class="col-md-9 col-sm-9 ">
                    <select name="supervisor" class="form-control" id="">
                      @foreach ($user as $u)
                        <option value={{ $u->id }} @if (old('supervisor') ? old('supervisor') == $u->id : $mandor->supervisor == $u->id) selected @endif>
                          {{ $u->username }}</option>
                      @endforeach
                    </select>
                    <small>
